...
<head>
  ...
  <title>Sign Up| Al Jazeera Media</title>
</head>
<body>
  <div class="wrapper">
      ...
      <nav class="navbar navbar-default firstnav">
        ...
        <div class="sidenav collapse navbar-collapse" id="myNavbar">
          ...
          <div class="menu-links">
            <ul class="menu-link">
              <li><a href="http://localhost/phpdesign/login.php">Login</a></li>|
              <li><a class="active" href="http://localhost/phpdesign/signup.php">Sign Up</a></li>
    ...
    <div class="col-sm-10 bg">
      <div>
        <section id="signupform" class="outer-wrapper">
                ...
                <div class="col-sm-4 col-sm-offset-4">
                  ...
                  <form role="form" method="post" action="<?php echo $_SERVER["PHP_SELF"];?>">
                    <div class="form-group inner-addon left-addon">
                      <i class="glyphicon glyphicon-envelope glyphicon-style"></i>
                      <input type="email" name="Email" class="form-control inp-style" placeholder="Email Address">
                    </div>
                    <div class="form-group inner-addon left-addon">
                    ...
                    <div class="form-group inner-addon left-addon">
                      <i class="glyphicon glyphicon-lock glyphicon-style"></i>
                      <input type="password" name="confirmpsw" class="form-control inp-style" placeholder="Confirm Password">
                    </div>
                    <div class="form-group">
                      <input type="submit" name="submit" class="btn-form" value="SIGN UP">
                    </div>
                    <p class="text-style text-center">You agree to the terms of services.</p>
                  </form>
    ...
    if(isset($_POST['submit'])){
      ...
      if ( (!empty($email)) && (!empty($username)) && (!empty($password)) && (!empty($confirmpsw)) ) {

        if ($password!=$confirmpsw) {
          ?>
          <p class="text-danger output-overimg">Password doesn't match</p>
          <?php
        }
        else {
          $check = mysqli_query($db, "SELECT * FROM users WHERE username='$username' or Email='$email'");
          if (mysqli_num_rows($check) != 0)
          {
            ?>
            <p class="text-danger output-overimg">The username or email already exists</p>
            <?php
            $check -> close();
          }
          else {

            $stmt = $db->prepare("INSERT INTO users(Email, username, password, confirmpsw) VALUES(?,?,?,?)");
            $stmt->bind_param("ssss",$email, $username, $password, $confirmpsw);
            $stmt->execute();
            ...
            {
              ?>
              <p class="text-success output-overimg">succesfully registered</p>
              <?php
            }
            else
            {
